...
class TrainingSnapshotAlignment(TrainingSnapshotAlignmentAbstractClass):
    ...
    def get_grid_align_indicates(self,x_min_b,y_min_b,R):

        embedding = self.projector.batch_project(self.ref_EPOCH, np.dot(self.ref_data_provider.train_representation(self.ref_EPOCH),R))
        x_min, y_min, x_max, y_max = self.visualizer.get_epoch_plot_measures(self.ref_EPOCH)
        # create grid
