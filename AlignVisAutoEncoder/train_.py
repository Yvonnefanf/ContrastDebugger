...
for epoch in range(num_epochs):
    ...
    for data_X, data_Y in dataloader: # Assuming you have a DataLoader instance with paired data (X, Y)
        ...
        complex_R, sigmas_t_R, rhos_t_R, knn_idxs_t_R = spatial_cons._construct_boundary_wise_complex(
            autoencoder.decoder(data_X).detach().numpy(),
            autoencoder.decoder(torch.Tensor(np.concatenate((ref_features,ref_neg_features),axis=0))).detach().numpy())
        complex, sigmas_t, rhos_t, knn_idxs_t = spatial_cons._construct_boundary_wise_complex(data_Y.detach().numpy(),np.concatenate((tar_features,tar_neg_features),axis=0))
        complex_2, sigmas_t_2, rhos_t_2, knn_idxs_t2 = spatial_cons._construct_boundary_wise_complex(transformed_Y.detach().numpy(),transformed_Y_b.detach().numpy())
        ...
        loss = loss_f_decoder + loss_f_encoder + 0.02 * pred_loss + 0.1 * cka_loss + 1e-5 * loss_complex
    ...
    'loss': loss
    }, os.path.join(TAR_PATH, "Model", "Epoch_{}".format(TAR_EPOCH),"test_use_complex{}.pth".format(epoch)))
