...
# input_X = np.concatenate((ref_features,ref_neg_features),axis=0)
# input_Y = np.concatenate((tar_features,tar_neg_features),axis=0)
...
pre_trained_model_loc = os.path.join(TAR_PATH, "Model", "Epoch_{}".format(TAR_EPOCH), "autoencoder.npy".format(6))
...
for epoch in range(num_epochs):
    ...
    for data_X, data_Y in dataloader: # Assuming you have a DataLoader instance with paired data (X, Y)
        ...
        emd_ = projector.batch_project(200,transformed_Y.detach().numpy())
        inv_Y = projector.batch_inverse(200, emd_)
        inv_Y = torch.Tensor(inv_Y)

        recon_X = autoencoder.decoder(inv_Y)
        ...
        # topological_loss_encoder = earth_movers_distance(data_Y,data_Y)
        
        loss_f_decoder = 10 * topological_loss_decoder
        ...
        cka_loss = cka_loss_f(data_Y,recon_X,transformed_Y)

        loss = loss_f_decoder + loss_f_encoder +  0.002 * pred_loss + 0.01 * cka_loss
    ...
    print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, Loss decoder: {loss_f_decoder},pred_loss,{pred_loss},CKA,{cka_loss}')
    torch.save({
    'epoch': TAR_EPOCH,
    'model_state_dict': autoencoder.state_dict(),
    'optimizer_state_dict': optimizer.state_dict(),
    'loss': loss
    }, os.path.join(TAR_PATH, "Model", "Epoch_{}".format(TAR_EPOCH),"autoencoder_OnDecoder{}.npy".format(epoch)))
