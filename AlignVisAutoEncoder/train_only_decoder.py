...
for epoch in range(num_epochs):
    ...
    for data_X, data_Y in dataloader_b: # Assuming you have a DataLoader instance with paired data (X, Y)
        ...
        recon_X = autoencoder.decoder(data_X)
        ...
        complex_2, sigmas_t_2, rhos_t_2, knn_idxs_t2 = spatial_cons._construct_fuzzy_complex(recon_X.detach().numpy())
        ...
        cka_loss = cka_loss_f(data_Y,recon_X,recon_X)

        loss =  0.5 * pred_loss + 0.1 * cka_loss + 0.001 * loss_complex
        ...
        # Update the weights
        optimizer.step()
        print(f'Epoch [{epoch+1}/{num_epochs}], Loss: {loss.item():.4f}, ,pred_loss,{pred_loss.item():.4f},CKA,{cka_loss.item():.4f},loss_complex:{loss_complex}')
    ...
    'loss': loss
    }, os.path.join(TAR_PATH, "Model", "Epoch_{}".format(TAR_EPOCH),"test_use_complex_decoder{}.pth".format(epoch)))
