...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, autoencoder, train_representation, projector, resolution, indicates, need_trans=True,cmap='tab10'):
    ...
    def get_epoch_decision_view(self, epoch, resolution):
        ...
        new_grid = np_grid_samples
        if self.need_trans:
            new_grid = self.autoencoder.decoder(torch.Tensor(np_grid_samples)).detach().numpy()
    ...
    def savefig(self, epoch, path="vis"):
        ...
        pred = self.data_provider.get_pred(epoch, after_trans)
        if len(self.indicates):
         
            after_trans = self.train_representation[self.indicates]
            pred = self.data_provider.get_pred(epoch, self.autoencoder.decoder(torch.Tensor(after_trans)).detach().numpy())
        ...
        for c in range(self.class_num):
            data = embedding[np.logical_and(pred == c, True)]
