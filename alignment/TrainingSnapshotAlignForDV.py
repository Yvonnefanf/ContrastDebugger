...
class TrainingSnapshotAlignment(TrainingSnapshotAlignmentAbstractClass):
    ...
    def prediction_loss(self, X, Y):
        prediction_X = self.tar_data_provider.get_pred(self.tar_EPOCH, X.detach().numpy())
        prediction_Y = self.tar_data_provider.get_pred(self.tar_EPOCH, Y.detach().numpy())
        loss_output = F.mse_loss(torch.tensor(prediction_X), torch.tensor(prediction_Y))
        loss_Rep = F.mse_loss(X, Y)
    ...
    def align_embeddings(self,X: np.ndarray, Y: np.ndarray,
        ...
        for i in range(train_steps):
            ...
            loss2 = self.prediction_loss(X.matmul(R),Y)
            loss3 = self.kernel_HSIC_cka_loss(X.matmul(R),Y)
            if i% 99 == 0:
                print(f"iteration {i}, loss1 {loss1}",'loss2', {loss2},'loss3',loss3) 
