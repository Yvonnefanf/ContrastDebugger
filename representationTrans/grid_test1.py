...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, R, RT_V, train_representation, projector, tar_projector, R_I,resolution, indicates, cmap='tab10'):
      
        self.data_provider = data_provider
        self.projector = projector
        self.tar_projector = tar_projector
    ...
    def get_grid(self, epoch, data, projector):
        resolution = 200
        data = data
        ...
        grid_samples = projector.batch_inverse(epoch, grid)
        np_grid_samples = np.asarray(grid_samples)

        return np_grid_samples
    ...
    def get_epoch_decision_view(self, epoch, resolution):
        ...
        new_grid = np.dot(np_grid_samples,self.R)




        tar_grid_samples = self.tar_projector.batch_inverse(epoch, grid)

        np_tar_grid_samples = np.asarray(tar_grid_samples)

        # ground_truth = self.data_provider.get_pred(200, data_provider.train_representation(200))
        ...
        border = np.zeros(len(diff), dtype=np.uint8) + 0.01
        border[diff < 0.15] = 1
        diff[border == 1] = 0.
