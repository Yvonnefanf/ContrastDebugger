...
class visualizer(VisualizerAbstractClass):
    def __init__(self, data_provider, R, pred_R, train_representation, projector,tar_projector,p_R, resolution, indicates, cmap='tab10'):
    ...
    def get_epoch_decision_view(self, epoch, resolution):
        ...
        np_tar_grid_samples = np.asarray(grid_samples)
        new_tar = np.dot(np_tar_grid_samples,self.R)


        
        mesh_preds = self.data_provider.get_pred(epoch, new_tar)
        # mesh_preds = np.dot(temp_preds,self.pred_R)
