...
noisy2500_provider = NormalDataProvider(NOISY2500, net, EPOCH_START, EPOCH_END, EPOCH_PERIOD, split=-1, device=DEVICE, classes=CLASSES,verbose=1)
...
noisy2500_data = noisy2500_provider.train_representation(200).squeeze()
...
import time
import json
filename = now = time.strftime("%Y-%m-%d-%H_%M_%Sexchange_Ref_clean.json", time.localtime(time.time())) 
