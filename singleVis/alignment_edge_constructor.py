...
class AlignmentEdgeConstructorAbstractClass(ABC):
    @abstractmethod
...
class AlignmentEdgeConstructor(AlignmentEdgeConstructorAbstractClass):
  
    def __init__(self, X, time_step_nums, sigmas, rhos, n_neighbors, n_epochs, persistent,time_step_idxs_list, knn_indices, ref_edge_to, ref_edge_from, ref_weight, ref_X, ref_knn_indices, ref_sigmas, ref_rhos, ref_provider, ref_projector) -> None:
...
class LocalAlignmentEdgeConstructor(AlignmentEdgeConstructor):
    def __init__(self, X, time_step_nums, sigmas, rhos, n_neighbors, n_epochs, persistent,time_step_idxs_list, knn_indices, ref_edge_to, ref_edge_from, ref_weight, ref_X, ref_knn_indices, ref_sigmas, ref_rhos, ref_provider, ref_projector) -> None:
        """
        construct temporal edges based on same data
        link data to its reference epoch
        ...
        ref_provider : reference data provider
            
        ref_projector : reference projector
        """
        super().__init__( X, time_step_nums, sigmas, rhos, n_neighbors, n_epochs, persistent,time_step_idxs_list, knn_indices, ref_edge_to, ref_edge_from, ref_weight, ref_X, ref_knn_indices, ref_sigmas, ref_rhos, ref_provider, ref_projector)
        self.persistence = persistent
        self.time_step_idxs_list = time_step_idxs_list
        self.knn_indices = knn_indices
        self.ref_provider = ref_provider
        self.ref_projector = ref_projector
        self.ref_knn_indices = ref_knn_indices

        

    
    def construct(self):
        """construct alignment edges
        ...
        for step in range(self.time_steps):
            ...
            # print("knn_indices_in",knn_indices_in)
           
            knn_dist = np.zeros((n_all, self.n_neighbors))

            ref_knn = self.ref_knn_indices[step]
            # print("ref",ref_knn)
            
            increase_idx = all_step_num[step]
            # print("increase_idx",increase_idx,all_step_num)
            knn_indices_in[step]=ref_knn
            knn_indices_in = knn_indices_in.astype('int')

            indices = np.arange(all_step_num[step], all_step_num[step] + self.time_step_nums[step][0], 1)[self.time_step_idxs_list[step]]
            knn_dists_t = knn_dists(self.features, indices, ref_knn)
        
            # knn_dist[all_step_num[step]:all_step_num[step] + time_step_nums[step + window][0]] = knn_dists_t\
            print("knn_dist",knn_dist,"knn_dists_t",knn_dists_t)
            assert len(knn_dists_t) == len(self.time_step_idxs_list[step])
            knn_dist = knn_dists_t
            # for i in range(len(self.time_step_idxs_list[step])):
            #     knn_dist[increase_idx + self.time_step_idxs_list[step][i]]=knn_dists_t[i]
        ...
        #     increase_idx = all_step_num[step]
        #     for i in range(len(self.time_step_idxs_list[step])):
        #         knn_indices_in[increase_idx + self.time_step_idxs_list[step][i]]=ref_knn[i]
        #     knn_indices_in = knn_indices_in.astype('int')
...
class GlobalAlignmentEdgeConstructor(AlignmentEdgeConstructor):
...
class GlobalParallelAlignmentEdgeConstructor(AlignmentEdgeConstructor):
